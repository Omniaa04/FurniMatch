  ...
  Future<void> addToCartAndOpenCart() async {
    ...
    try {
      ...
      if (success) {
        ...
        // ✅ setUserId هنا بس — بدون LoadCartEvent
        // CartPage هي اللي هتعمل LoadCartEvent في initState بتاعها
        // لو بعتنا LoadCartEvent هنا، هيحصل race condition:
        // الـ Bloc بيجيب items، وبعدين CartPage بتفتح وتبعت تاني LoadCartEvent
        // وده بيعمل CartLoading تاني → فاضي مؤقتاً → يظهر empty
        sl<CartLocalDataSource>().setUserId(widget.userId!);
