...
class MeasurementResultScreen extends StatefulWidget {
  final MeasurementResultEntity result;
...
Future<void> _saveImageAndRoom() async {
  ...
  try {
    ...
    // 2) روح لـ RoomNameScreen مع الـ dimensions
    ...
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
        builder: (_) => RoomNameScreen(dimensions: dims),
      ),
    );
  } catch (e) {
    if (mounted) {
...
class _AnnotationPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (n == 2) {
      ...
      final color =
          widthM != null ? const Color(0xFF00E5FF) : RoomAppColors.secondary;
    ...
    if (n == 3) {
      ...
      final dxArm1 =
          (wallPoints[(cornerIdx + 1) % 3].dx - wallPoints[cornerIdx].dx).abs();
      final dyArm1 =
          (wallPoints[(cornerIdx + 1) % 3].dy - wallPoints[cornerIdx].dy).abs();
  ...
  void _drawLine(Canvas canvas, Offset a, Offset b, String label, Color color) {
    final dir = b - a;
  ...
  void _label(Canvas canvas, Offset center, String text, Color color) {
    ...
    final cx = center.dx
        .clamp(displayRect.left + rw / 2 + 4, displayRect.right - rw / 2 - 4);
    final cy = center.dy
        .clamp(displayRect.top + rh / 2 + 4, displayRect.bottom - rh / 2 - 4);
    ...
    canvas.drawRRect(
        ...
        Paint()
          ...
          ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 8));
    canvas.drawRRect(
        rr, Paint()..color = const Color(0xFF121212).withValues(alpha: 0.85));
    canvas.drawRRect(
        rr,
        Paint()
          ..color = Colors.white.withValues(alpha: 0.15)
