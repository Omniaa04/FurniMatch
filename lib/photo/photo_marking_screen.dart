...
class _PhotoMarkingScreenState extends State<PhotoMarkingScreen>
  ...
  Future<void> _saveDebugInfo(MeasurementResult r) async {
    try {
      final buffer = StringBuffer();
      buffer.writeln('\n${'=' * 40}');
      buffer.writeln('<<<< DEBUG DATA START >>>>');
      buffer.writeln('=== Measurement Debug Info ===');
      buffer.writeln('Timestamp: ${DateTime.now()}');
      buffer.writeln('Image Path: ${r.imagePath}');
      buffer.writeln('Image Size: ${r.imageSize.width.toInt()} x ${r.imageSize.height.toInt()}');
      
      buffer.writeln('\n--- Reference Object ---');
      buffer.writeln('Name: ${widget.reference.name}');
      buffer.writeln('Ref Width: ${widget.reference.widthMm} mm');
      buffer.writeln('Ref Height: ${widget.reference.heightMm} mm');
      
      buffer.writeln('\n--- Phase 1: Reference Points (Pixels) ---');
      buffer.writeln('// Copy this into your test/debug script:');
      buffer.writeln('final refPoints = [');
      for (int i = 0; i < r.refPoints.length; i++) {
        final p = r.refPoints[i];
        buffer.writeln('  Offset(${p.dx.toStringAsFixed(2)}, ${p.dy.toStringAsFixed(2)}),');
      }
      buffer.writeln('];');

      buffer.writeln('\n--- Phase 2: Surface Points (Pixels) ---');
      buffer.writeln('final wallPoints = [');
      for (int i = 0; i < r.wallPoints.length; i++) {
        final p = r.wallPoints[i];
        buffer.writeln('  Offset(${p.dx.toStringAsFixed(2)}, ${p.dy.toStringAsFixed(2)}),');
      }
      buffer.writeln('];');

      buffer.writeln('\n--- Calculated Results ---');
      buffer.writeln('Width:  ${r.widthM.toStringAsFixed(4)} m');
      buffer.writeln('Height: ${r.heightM.toStringAsFixed(4)} m');
      buffer.writeln('<<<< DEBUG DATA END >>>>');
      buffer.writeln('=' * 40 + '\n');
      
      final output = buffer.toString();
      
      // 1. Always print to console for development environment access
      debugPrint(output);

      // 2. Also keep the local file save on the device just in case
      final debugPath = r.imagePath.replaceAll(RegExp(r'\.[^.]+$'), '_debug.txt');
      await File(debugPath).writeAsString(output);
      
    } catch (e) {
      debugPrint('Debug output failed: $e');
    }
  }
  */
            ...
            : (d) {
                ...
                setState(() {
                  _activePoints[i] = _activePoints[i] +
                      Offset(
                        (d.delta.dx * precisionFriction) / totalScale, 
              ...
              if (!isRef) ...[
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      _modeChip('2 pts', 'Distance', Icons.linear_scale_rounded),
                      const SizedBox(width: 8),
                      _modeChip(
              ...
              Padding(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: isRef ? AppColors.primary : AppColors.secondary,
                      borderRadius: BorderRadius.circular(16),
                    ),
                    child: const Text(
                      'Got it — tap to start',
